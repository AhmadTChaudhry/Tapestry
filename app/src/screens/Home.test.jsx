...
describe('Home', () => {
  ...
  it('shows an empty state and the import affordances when nothing is in progress', () => {
    ...
    const sampler = screen.getByRole('img', { name: 'A playful stitch sampler' })
    expect(sampler).toBeVisible()
    expect(sampler.querySelector('[stroke-dasharray]')).toBeNull()
    expect(sampler.querySelector('circle')).toBeNull()
    const stitchPaths = sampler.querySelectorAll('path[transform]')
    expect(stitchPaths).toHaveLength(25)
    expect([...stitchPaths].every(path => path.getAttribute('d')?.startsWith('M1 1'))).toBe(true)
